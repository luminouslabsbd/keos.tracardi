...
function checkUrlRoleMapping() {
    ...
    for (var i = 0; i < jsonUrls.length; i++) {
        ...
        if (
            ...
            loadScript(
                "http://localhost/assets/js/eventos_domain.tracardi.js",
                scriptLoaded
            );
...
var url = "http://localhost/assets/json/eventos_domain.json";
...
function handleMessageFromChild(event) {
    ...
    if (isEventOriginInJsonData(window.location.href)) {
        ...
        if (messageData.type === "FormSubmit") {
            ...
            loadScript("http://localhost/assets/js/test.tracardi.js", scriptLoaded);
        }
    } else {
