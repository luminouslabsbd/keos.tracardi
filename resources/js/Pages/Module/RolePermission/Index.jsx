...
function Index() {
            ...
            <div>
                {data.map((item, index) => (
                    <React.Fragment key={index}>
                        <span className="badge badge-outline-success ml-2">{item?.name}</span>
    ...
    const columns = useMemo(
        () => [
            ...
            {
                header: 'Permissions',
                Cell: ({ row }) => <Permission data={row.original?.permissions} />,
            },
            {
                header: 'Actions',
                Cell: ({ row }) => (
    ...
    return (
        <>
            ...
            <DeleteModal isDeleteNoteModal = {isDeleteNoteModal} setIsDeleteNoteModal={setIsDeleteNoteModal} fileToDelete={fileToDelete} name="Role Permission" route="role-permission"></DeleteModal>
        </>
    );
}

Index.layout = (page) => (
    <MainLayout children={page} title="My Student || List" />
);
