...
function Index() {
    ...
    const { sources, base_url } = usePage().props;

    const handleCreate = () => {
        router.get(`/admin/event-sources/create`);
    };

    const handleEdit = (id) => {
        router.get(`/admin/event-sources/edit/${id}`);
    };

    const handleDelete = (id) => {
        if (confirm("Are you sure you want to delete this domain?")) {
            router.delete(`/admin/event-sources/delete/${id}`);
        }
    };

    const handleUrlCreate = (id) => {
        router.get(`/admin/event-sources/domain-url/create/${id}`);
    };

    const handleDetails = (id) => {
        router.get(`/admin/event-sources/details/${id}`);
    };

    const handleCheckboxChange = async (id, checked) => {
        try {
            const updatedStatus = checked ? 1 : 0;
            router.post("/admin/event-sources/status", {
                id,
                ...
                <div className="col-span-12 pt-4">
                    <div className="panel">
                        <div className="mb-2 flex justify-between items-center">
                            <h5 className="font-bold">Event source list</h5>
                        </div>
                        <hr />
